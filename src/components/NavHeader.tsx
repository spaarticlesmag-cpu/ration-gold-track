...
export const NavHeader = ({ userName, userRole }: NavHeaderProps) => {
  ...
  const { profile, signOut, devMode, devSignOut } = useAuth();
  const { totalItems } = useCart();
  const [notifications] = useState(0); // Set to 0 for unauthenticated users
  ...
  const handleLogout = async () => {
    try {
      if (devMode) {
        devSignOut();
        // Immediately redirect to landing page for dev mode
      ...
      <div className="container mx-auto px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <Link to="/" className={`text-2xl font-bold ${branding.color} bg-gradient-to-r from-amber-600 to-yellow-500 bg-clip-text text-transparent hover:scale-105 transition-transform`}>
              JADAYU
            </Link>
                ...
                <>
                  ...
                  <Button asChild variant={location.pathname === '/quota' ? 'default' : 'ghost'}>
                    <Link to="/quota" className="flex items-center gap-2">
                      <Package className="icon-lg" /> Quota
                    </Link>
                  </Button>
          ...
          <div className="flex items-center space-x-4">
            {/* Show auth buttons for unauthenticated users, or profile/cart for authenticated users */}
            {!isAuthenticated ? (
              <div className="hidden md:flex items-center space-x-4">
                <Button asChild className="bg-amber-600 text-white shadow-lg">
